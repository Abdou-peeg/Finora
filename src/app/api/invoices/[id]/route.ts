...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const inv = await db.invoice.findFirst({ where: { id: params.id, tenantId } });
  if (!inv) return NextResponse.json({ error: "Facture introuvable" }, { status: 404 });
  ...
  try {
    if (body.action === "pay") {
      ...
      const updated = await payInvoice(params.id, amount, g.user);
      return NextResponse.json(updated);
    }
    if (body.action === "cancel") {
      const updated = await db.invoice.update({ where: { id: params.id }, data: { status: "CANCELLED" } });
      await db.auditLog.create({
        data: {
          tenantId, userId: g.user.id, userName: g.user.name,
          action: "INVOICE_CANCELLED", entity: "Invoice", entityId: params.id,
