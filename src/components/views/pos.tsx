...
export function PosView() {
  ...
  function resetSale() { setCart([]); setCustomerId(""); setCustomCustomerName(""); setDiscount(""); setReceived(""); setQuery(""); setCategory("Toutes"); }
  async function checkout() {
    ...
    try {
      // If there's a discount, we apply it proportionally to each item to keep the backend totals consistent
      // since the current API doesn't support a global discount field.
  ...
  return <div className="space-y-4 print:hidden">
    ...
    <div className="grid min-h-[calc(100vh-13rem)] gap-4 xl:grid-cols-[minmax(0,1fr)_410px]">
      <section className="rounded-xl border bg-card p-4 shadow-sm">
        ...
        {productsLoading ? <div className="py-16 text-center text-sm text-muted-foreground">Chargement des produits…</div> : visibleProducts.length === 0 ? <div className="py-16 text-center text-sm text-muted-foreground">Aucun produit trouvé.</div> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5">{visibleProducts.map((product: any) => <button key={product.id} type="button" onClick={() => addProduct(product)} className="flex flex-col overflow-hidden rounded-xl border bg-background text-left shadow-sm transition active:scale-[.98] hover:border-primary hover:bg-primary/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
      ...
      <aside className="flex flex-col rounded-xl border bg-card shadow-sm">
        <div className="border-b p-4"><div className="flex items-center justify-between"><h2 className="font-semibold">Panier <span className="text-muted-foreground">({cart.reduce((sum, line) => sum + line.qty, 0)})</span></h2>{cart.length > 0 && <Button variant="ghost" size="sm" className="text-rose-600" onClick={() => setCart([])}><Trash2 className="mr-1 h-4 w-4" /> Vider</Button>}</div><Button variant="outline" className="mt-3 h-11 w-full justify-start" onClick={() => setCustomerPicker(true)}><UserRound className="mr-2 h-4 w-4" />{selectedCustomer ? selectedCustomer.name : (customCustomerName || "Comptant")}</Button></div>
        ...
        <div className="space-y-3 border-t p-4"><div className="space-y-1 text-sm"><div className="flex justify-between text-muted-foreground"><span>Sous-total HT</span><span>{currency(round2(totals.subtotal))}</span></div><div className="flex justify-between text-muted-foreground"><span>TVA</span><span>{currency(round2(totals.tax))}</span></div>{discountValue > 0 && <div className="flex justify-between text-rose-600"><span>Réduction</span><span>-{currency(discountValue)}</span></div>}<div className="flex justify-between pt-1 text-xl font-bold"><span>Total TTC</span><span className="text-primary">{currency(total)}</span></div></div><div className="grid grid-cols-2 gap-2"><div><label className="mb-1.5 block text-xs font-medium text-muted-foreground">Réduction (Remise)</label><Input inputMode="decimal" value={discount} onChange={(e) => setDiscount(e.target.value)} placeholder="0" className="h-10 text-rose-600 font-semibold" /></div><div><label className="mb-1.5 block text-xs font-medium text-muted-foreground">Espèces reçues</label><Input inputMode="decimal" value={received} onChange={(e) => setReceived(e.target.value)} placeholder={total ? String(total) : "0"} className="h-10 font-semibold" /></div></div><div className={cn("flex justify-between text-sm", receivedNumber >= total ? "text-emerald-600" : "text-muted-foreground")}><span>Monnaie à rendre</span><strong>{currency(change)}</strong></div></div><Button className="h-14 w-full text-lg font-bold" disabled={!cart.length || createSale.isPending} onClick={checkout}>{createSale.isPending ? "Encaissement…" : "Encaisser"}</Button></div>
      </aside>
    </div>
    <Dialog open={customerPicker} onOpenChange={setCustomerPicker}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>Client de la vente</DialogTitle></DialogHeader><div className="space-y-4"><div className="space-y-2"><label className="text-sm font-medium">Nom du client (saisie libre)</label><div className="flex gap-2"><Input value={customCustomerName} onChange={(e) => { setCustomCustomerName(e.target.value); if (e.target.value) setCustomerId(""); }} placeholder="Ex: Jean Dupont..." /><Button variant="outline" onClick={() => setCustomCustomerName("")}>Effacer</Button></div></div><div className="relative"><div className="absolute inset-0 flex items-center"><span className="w-full border-t" /></div><div className="relative flex justify-center text-xs uppercase"><span className="bg-background px-2 text-muted-foreground">Ou sélectionner un client existant</span></div></div><Input value={customerQuery} onChange={(e) => setCustomerQuery(e.target.value)} placeholder="Rechercher dans la base…" /><div className="max-h-60 space-y-1 overflow-y-auto"><Button variant={!customerId && !customCustomerName ? "secondary" : "ghost"} className="h-12 w-full justify-start" onClick={() => { setCustomerId(""); setCustomCustomerName(""); setCustomerPicker(false); }}>Comptant (Anonyme)</Button>{customers.filter((customer: any) => `${customer.code} ${customer.name}`.toLowerCase().includes(customerQuery.toLowerCase())).map((customer: any) => <Button key={customer.id} variant={customerId === customer.id ? "secondary" : "ghost"} className="h-12 w-full justify-start" onClick={() => { setCustomerId(customer.id); setCustomCustomerName(""); setCustomerPicker(false); }}><span className="mr-2 font-mono text-xs text-muted-foreground">{customer.code}</span>{customer.name}</Button>)}</div></div></DialogContent></Dialog>
